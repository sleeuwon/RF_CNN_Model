...
def train_model(train_dataloader, val_dataloader, model, criterion, optimizer, num_epochs, device):
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, outputs in train_dataloader:
            ...
            for i in range(inputs.size(1)):  # Loop over outputs
                for j in range(inputs.size(2)):  # Loop over frequencies within each output
                    current_loss = criterion(predictions_recon[:, i, j], inputs[:, i, j])
                    loss_recon += current_loss
                    train_losses_per_frequency_recon[j].append(current_loss.item())  # Append loss for this frequency
            ...
            #loss.backward()#reapplying the backward for better optimization?
            optimizer.step()
        ...
        #for j in range(num_frequencies):  # Loop over frequencies
            #train_loss_per_frequency = np.mean(train_losses_per_frequency[j]) if train_losses_per_frequency[j] else float('nan')
            #val_loss_per_frequency = np.mean(val_losses_per_frequency[j]) if val_losses_per_frequency[j] else float('nan')
            #print(f"Frequency {j+1}, Average Train Loss: {train_loss_per_frequency:.4f}, Average Val Loss: {val_loss_per_frequency:.4f}")
    ...
    #with open("final_epoch_losses_Softmarginloss_comparison.pkl", "wb") as f:
    ...
    #with open("training_results_MAE_4.pkl", "wb") as f:
